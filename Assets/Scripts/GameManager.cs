...
public class GameManager : MonoBehaviour
{
    ...
    public void WatchAdForSuperFood() { pendingAdReward = AdRewardType.SuperFood; if (YandexManager.Instance != null) YandexManager.Instance.ShowRewardAd(); }
    public void WatchAdForDoubleScore() { pendingAdReward = AdRewardType.DoubleScore; if (YandexManager.Instance != null) YandexManager.Instance.ShowRewardAd(); }
    private void OnAdRewarded() { if (pendingAdReward == AdRewardType.SuperFood) SuperFeedCat(); else if (pendingAdReward == AdRewardType.DoubleScore) { if (score > 0) score *= 2; else score += 100; currentDoubleScoreTimer = doubleScoreCooldown; } pendingAdReward = AdRewardType.None; UpdateAllUITexts(); }
    ...
    public void OnCatClicked(BaseEventData baseData) { if (!this.enabled) return; PointerEventData e = baseData as PointerEventData; if (e == null) return; HandleClick(e.position); }
    private void HandleClick(Vector2 clickPosition) { AudioManager.Instance.PlaySound(catClickSound); double finalScorePerClick = scorePerClick * clickMultiplier; score += finalScorePerClick; CheckForLevelUp(); catImage.transform.localScale = new Vector3(1.1f, 1.1f, 1f); CancelInvoke("ResetCatScale"); Invoke("ResetCatScale", 0.1f); if (clickTextPrefab != null && canvasTransform != null) { GameObject textGO = Instantiate(clickTextPrefab, canvasTransform); RectTransform canvasRect = canvasTransform.GetComponent<RectTransform>(); Vector2 localPoint; Camera cam = (uiCamera != null) ? uiCamera : Camera.main; RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRect, clickPosition, cam, out localPoint); textGO.GetComponent<RectTransform>().localPosition = localPoint; textGO.GetComponent<TextMeshProUGUI>().text = "+" + FormatNumber(finalScorePerClick); } }
    ...
    private void ApplyLevelUp(bool playEffects = true)
    {
        ...
        if (playEffects && YandexManager.Instance != null) YandexManager.Instance.ShowInterstitialAd();
